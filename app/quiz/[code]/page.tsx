...
export default function QuizPage() {
    ...
    queryFn: async () => {
      ...
      if (!json.ok) {
        if (json.status === 'COMPLETED' || json.status === 'FAILED') {
          router.replace(`/quiz/${code}/review`);
        }
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`/api/quiz/${code}/submit`, {
        method: 'POST',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!data) return;
    const unanswered = data.questions.find((q) => !answers[q.id]);
    if (unanswered) {
  ...
  return (
    <div className="px-6 py-10">
      <header className="mb-8 rounded-2xl bg-primary-600 px-6 py-5">
        <h1 className="text-2xl font-bold text-white">다나 멘토링 퀴즈</h1>
        <p className="mt-1.5 text-sm text-primary-200">{data.questions.length}문항</p>
      </header>
      ...
      <form onSubmit={handleSubmit} noValidate>
          ...
          {data.questions.map((question, qIdx) => (
            <div key={question.id} className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
              <p className="mb-4 text-sm font-semibold text-neutral-900 leading-relaxed">
                <span className="mr-2 text-primary-600">Q{qIdx + 1}.</span>
                {question.title}
              </p>
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          <p className="text-sm text-neutral-500">
            {answeredCount} / {data.questions.length} 문항 답변 완료
          </p>
          ...
          <button
            ...
            className="w-full rounded-xl bg-primary-600 py-3 text-sm font-semibold text-white transition-colors hover:bg-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
